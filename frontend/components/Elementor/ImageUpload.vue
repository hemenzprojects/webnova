<template>
  <div>
    ...
    <div class="space-y-3">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          type="button"
          :disabled="uploading"
          class="px-3 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition flex items-center gap-2"
        >
        ...
        <button
          @click="mediaLibrary?.open()"
          type="button"
          class="px-3 py-2 bg-white text-gray-700 border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 transition flex items-center gap-2"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <input
          ...
          class="flex-1 min-w-32 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      </div>
    ...
    <!-- Media Library modal -->
    <MediaLibrary ref="mediaLibrary" @select="onMediaSelect" />
  </div>
</template>
...
const mediaLibrary = ref<{ open: () => void } | null>(null)
...
const currentImageUrl = computed(() => {
  ...
  return getImageUrl(props.modelValue) || null
})
